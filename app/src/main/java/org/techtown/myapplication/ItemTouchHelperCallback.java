...
public class ItemTouchHelperCallback extends ItemTouchHelper.Callback{
    ...
    public ItemTouchHelperCallback(ListAdapter listener){
    ...
    { //아이템이 스와이프 됐을경우 버튼을 그려주기 위해서 스와이프가 됐는지 확인
        ...
        if(buttonsShowedState == ButtonsState.GONE){ super.onChildDraw(c, recyclerView, viewHolder, dX, dY, actionState, isCurrentlyActive); } }

        currenrtItemViewHolder = viewHolder;

//버튼을 그려주는 함수
drawButtons(c, currenrtItemViewHolder); } private void drawButtons(Canvas c, RecyclerView.ViewHolder viewHolder)
    ...
    //오른쪽으로 스와이프 했을때 (왼쪽에 버튼이 보여지게 될 경우)
        ...
        drawText("수정", c, leftButton, p); buttonInstance = leftButton;
    ...
    private void setTouchDownListener
       (final Canvas c, final RecyclerView recyclerView ,
        ...
        final boolean isCurrentlyActive){ recyclerView.setOnTouchListener(new View.OnTouchListener()
    { @Override public boolean onTouch(View v, MotionEvent event)
    ...
    recyclerView.setOnTouchListener(new View.OnTouchListener() { @Override public boolean onTouch(View v, MotionEvent event)
    { return false; } }); setItemsClickable(recyclerView, true); swipeBack = false;
